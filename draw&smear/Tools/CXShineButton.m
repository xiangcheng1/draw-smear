...
CGFloat const KcycleMinPercent = 0.2;
...
@interface CXShineButton ()<CAAnimationDelegate>
// property
@property (nonatomic,copy) cxActionBlock normalAction;
@property (nonatomic,copy) cxActionBlock hightlightAction;
...
@property (nonatomic,strong) CXAnimateLineContainerView *lineContainerView;
@property (nonatomic,strong) CAShapeLayer * maskLayer;
// tag
@property (nonatomic,assign) BOOL isAnimating;
@property (nonatomic,assign) BOOL isHightlight;
@end
@implementation CXShineButton
...
-(void)layoutSubviews
{
    ...
    UIImage * currImage = self.isHightlight ? self.hightlightImage : self.normalImage;
...
{
    CXShineButton * button = [[CXShineButton alloc] init];
    button.shineTintColor = tintColor;
    button.normalImage = normalImage;
    button.hightlightImage = hightlightImage;
    button.normalAction = normalAction;
    button.hightlightAction = hightlightAction;
...
-(void)reloadStateWithNormalImage:(UIImage *)normalImage hightlightImage:(UIImage *)hightlightImage tintColor:(UIColor *)tintColor
{
    ...
    self.hightlightImage = hightlightImage;
...
- (void)p_animateStep1
{
    // animating if have hight action dealing
    if (self.isAnimating) {
        if (self.isNeedDealingActions) {
            if (self.hightlightAction) {
                self.hightlightAction();
            }
        }
        return;
    }
    ...
    animateStep1.fromValue = (id)[UIBezierPath bezierPathWithOvalInRect:[self p_createOvalWithPercent:KcycleMinPercent]].CGPath;
    animateStep1.toValue = (id)[UIBezierPath bezierPathWithOvalInRect:[self p_createOvalWithPercent:KcycleMinPercent]].CGPath;;
...
- (CGRect)p_createOvalWithPercent:(CGFloat)percent {
    CGFloat weightHeight = self.width * percent;
    CGFloat x = (self.width - weightHeight) / 2;
    CGFloat y = (self.width - weightHeight) / 2;
...
- (UIView *)lineContainerView {
    if (_lineContainerView == nil) {
        _lineContainerView = [[CXAnimateLineContainerView alloc]init];
...
@interface CXAnimateLineContainerView ()<CAAnimationDelegate>

@property (nonatomic,strong) NSMutableArray * lineArray;

@end

@implementation CXAnimateLineContainerView
