...
@implementation CXDoodleMenuBar
...
-(void)animationSHowDoodleMenuBarWithAnchorPoint:(CGPoint)anchorPoint
{
    ...
    // Foot circle frame
    self.footCircle.center = CGPointMake(anchorPoint.x, anchorPoint.y - 10);
    ...
    // Semi circle frame
    self.semicircle.center = CGPointMake(anchorPoint.x - KSemiCircleSize/2, anchorPoint.y - 25);
    ...
    CGFloat toolBarStartCenterX = (anchorPoint.x + KToolBarHeight/2) - toolBarStartW/2;
    ...
    CGFloat toolBarEndCenterX = (anchorPoint.x + KToolBarHeight/2) - toolBarEndW/2;
    ...
    [self.menuItemArray enumerateObjectsUsingBlock:^(CXDoodleMenuButtonItem *item, NSUInteger idx, BOOL * _Nonnull stop) {
        @strongify(self);
        ...
        CXShineButton *btn = [CXShineButton shineButtonWithTintColor:item.selectColor normalImage:normalImage hightlightImage:selectImage normarTypeTapAction:^{
            [self.delegate doodleMenuBarClickIndex:idx barItemIsSelectAfter:YES];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self p_animateToDismissMenuBar];
            });
        } hightlightTypeTapAction:^{
            [self.delegate doodleMenuBarClickIndex:idx barItemIsSelectAfter:NO];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self p_animateToDismissMenuBar];
            });
        }];
...
{
    CXDoodleMenuBar * bar = [[CXDoodleMenuBar alloc] init];
...
////////////////////////////////////////////////////////
@implementation CXDoodleMenuButtonItem
...
{
    CXDoodleMenuButtonItem *item = [[CXDoodleMenuButtonItem alloc]init];
